  ...
  return new Promise((resolve) => {
    ...
    proc.on("close", (code) => {
      if (code === 0) resolve(true);
        ...
        console.warn(
          `Command ${command} failed with code ${code}. Skipping...`,
        );
        resolve(false);
      }
    });
    proc.on("error", (err) => {
      console.warn(
        `Command ${command} not found or failed to start. Skipping...`,
      );
      resolve(false);
    });
  });
...
async function optimizeTextures() {
  ...
  const presets: [RegExp, string][] = [
    ...
    [
      /career\/.+png$/,
      "--nowarn --2d --t2 --encode uastc --assign_oetf srgb --target_type RG",
    ],
  ...
  for (const file of files) {
    ...
    try {
      ...
      await runCommand("toktx", [...preset.split(" "), output, input]);
      console.log(`Optimized Texture: ${file} -> .ktx2`);
    } catch (err) {
      console.warn(
        `Skipping KTX2 for ${file} (toktx not found or failed). Fallback to WebP if needed.`,
      );
  ...
  for (const styleFile of stylesheetFiles) {
    ...
    const newContent = content.replace(
      ...
      (match, body) => {
        ...
        if (!woff2Match) {
          const otherMatch = body.match(
            /url\(['"]?(.+?)\.(ttf|woff|eot|svg)['"]?\)/,
          );
          if (otherMatch) {
