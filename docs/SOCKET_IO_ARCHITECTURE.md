...
### E. WebRTC Signaling (Production-Ready)

To ensure reliable peer-to-peer connections, the system implements a stateful signaling server using Redis:

1.  **Offer Caching:** Active WebRTC offers are stored in a Redis Hash (`webrtc_offers`). This allows the system to handle asynchronous connections where the answerer might join slightly after the offer is created.
2.  **ICE Backlog Management:** ICE candidates generated by the offerer are cached in Redis.
3.  **Atomic Handshake:** The `call-answer` event uses a Socket.io acknowledgment. When a peer answers, the server atomically returns the entire backlog of cached ICE candidates, ensuring the `RemoteDescription` is set before candidates are added, thus preventing race conditions.
4.  **Automatic Cleanup:** Signaling state is strictly tied to the call lifecycle and is automatically purged upon call completion or decline.
