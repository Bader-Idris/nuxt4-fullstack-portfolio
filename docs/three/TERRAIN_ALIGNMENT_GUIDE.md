...
- **World Origin:** The center of your terrain must be at `(0, 0, 0)`.
  - _Fix:_ Select Terrain -> `Object` -> `Set Origin` -> `Origin to Geometry`.
  - _Fix:_ Set Location to `(0, 0, 0)`.
  - _CRITICAL:_ Apply all transforms (`Ctrl+A` -> `All Transforms`).
...
- _Reference:_ `app/composables/threeD/useTerrain.ts` (Lines 11, 74, 105)
...
- _Reference:_ `app/components/three/shaders/grass/vertex.glsl` (Line 92)
- _Reference:_ `app/composables/threeD/useTerrain.ts` (Line 105)
