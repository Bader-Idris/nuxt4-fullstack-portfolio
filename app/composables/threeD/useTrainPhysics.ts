...
export function useTrainPhysics() {
  ...
  function addWheelColliders(
    ...
    const meshes =
      wheelMeshesRef.meshes.length > 0
        ? wheelMeshesRef.meshes
        : collectWheelMeshes(locomotive);

    if (meshes.length === 0) {
      console.warn(
        "[useTrainPhysics] addWheelColliders: no wheel meshes found – call collectWheelMeshes() first",
      );
      return [];
    }
    ...
    for (const { mesh, wheel } of meshes) {
      ...
      const halfWidth = wheel.isBigWheel ? 0.14 : 0.1;

      const colliderDesc = rapier.ColliderDesc.cylinder(halfWidth, wheel.radius)
    ...
    console.log(
      `[useTrainPhysics] addWheelColliders: attached ${colliders.length} wheel colliders`,
    );
    return colliders;
  }
  ...
  function rotateWheelsByDistance(
    ...
    for (const { mesh, wheel, initialRotation } of wheelMeshes) {
      ...
      const spinDirection = wheel.spinDirection ?? 1;
      const rotationDelta =
        (Math.abs(distanceDelta) / wheel.radius) * Math.PI * 2;
      const signedDelta =
        rotationDelta * spinDirection * (distanceDelta >= 0 ? -1 : 1);
  ...
  function castRayAny(
    ...
    const hit = world.castRay(
      ...
      undefined,
      (collider) => {
        if (excludeBody && collider.parent()?.handle === excludeBody.handle)
          return false;
        return true;
      },
    );
